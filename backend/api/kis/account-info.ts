...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    const { password, isPaper = false } = req.body;
    ...
    // 계좌 잔고 조회
    const accountData = await kisClient.getAccountBalance();

    // 리소스 정리
    kisClient.destroy();

    // 성공 응답
    res.status(200).json(createApiResponse(true, accountData, '계좌 정보 조회 성공'));
